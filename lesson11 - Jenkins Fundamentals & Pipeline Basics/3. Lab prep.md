# Lesson 11: Jenkins Fundamentals & Pipeline Basics - Lab Preparation

### 1. Generate DockerHub PAT and GitHub PAT (personal access token)

- Create file `.env` from `.env.example`
- Replace env with your generated PAT
...
- Go to `manage jenkins` -> agent -> docker-agent -> copy the jenkins secret and paste to `JENKINS_SECRET` agent environment variable in docker-compose.yaml. after that, run `docker compose up -d`
